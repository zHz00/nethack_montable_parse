...
{"M3_GROUPATTACK",  0x0400},      /* hits and runs in groups*/
{"M3_BLINKER",      0x0800},      /* alternates between regenerating and attacking. */
{"M3_NOREGEN",      0x1000},      /* does not naturally regenerate hp. */
{"M3_STATIONARY",   0x2000},      /* does not move. */
{"M3_DISPLACES",0x4000},
{"",0}
...
experience(struct permonst *ptr)
{
    ...
    if
        (
            strcmp(ptr->mname,"fern spore")==0||
            strcmp(ptr->mname,"arctic fern spore")==0||
            strcmp(ptr->mname,"blazing fern spore")==0||
...
int main()
{
    ...
    std::sprintf(fname,"..\\mondump-%s-un6.csv",timeString);
    ...
    while(mons[x].mname[0]!='\0')
    {
        ...
        if(strcmp(mons[x].mname,"evil eye")==0)
            fout<<"LUCK=100|";
        if(mons[x].mflags1&M1_ACID)
            fout<<"UNSTONE=100|";
        if(strcmp(mons[x].mname,"chameleon")==0||
           strcmp(mons[x].mname,"sandestin")==0||
           strcmp(mons[x].mname,"doppelganger")==0)
            fout<<"POLYSELF=100|";
        if(strcmp(mons[x].mname,"mind flayer")==0||strcmp(mons[x].mname,"master mind flayer")==0)
            fout<<"GAIN_IN=50|";
        ...
        /* 28 -- eat danger */
        //fout<<"-,";
        if(strcmp(mons[x].mname,"disenchanter")==0)
            fout<<"CURSE_ATTR=100|";
