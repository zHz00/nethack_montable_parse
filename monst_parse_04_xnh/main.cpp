...
int main()
{
    ...
    fout<<"xNetHack 9.0,namem,namef,namen,symbol,Difficulty Lvl,Move Rate,Armor Class,";
    //     0     1     2     3     4      5              6         7
    ...
    while(mons[x].pmnames[NEUTRAL][0]!='\0')
    {
        ...
        int y=0;
        do{
            if(strcmp(mons[x].pmnames[NEUTRAL],trolls[y])==0)
            {
        ...
        if(strcmp(mons[x].pmnames[NEUTRAL],"disenchanter")==0)
            fout<<"CURSE_ATTR=100|";
        if(strcmp(mons[x].pmnames[NEUTRAL],"black mold")==0)
            fout<<"ILL=100|";
